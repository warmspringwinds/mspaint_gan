...
Z = torch.nn.Parameter( torch.randn(1, 512, device=device) )
OPTIMIZER = torch.optim.SGD((Z,), lr=100000.0)
...
def update_photo(data=None,widget=None):
    ...
    if data is None:
        ...
        with torch.no_grad():
            ...
            data = (img_normalized.cpu().squeeze(0).detach().numpy()[:, ::4, ::4] * 255).astype(np.uint8)
...
def Reset():
    ...
    Z = torch.FloatTensor(final_w_vectors)
    Z = Z.to(device)
    Z = torch.nn.Parameter( Z )
    
    #Z = torch.nn.Parameter( torch.randn(1, 512, device=device) )
    OPTIMIZER = torch.optim.SGD((Z,), lr=0.00005)
